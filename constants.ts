...
export const DEFAULT_REB_CONTEXT = `
  ...
  3. TELEMETRY FORMAT SPECIFICATION (MANDATORY)
     A. DELTA: <!-- Δ Ar±XX Ox±XX Fv±XX En±XX PC_AL±XX PC_AW±XX PC_OB±XX REB_AL±XX REB_AW±XX REB_OB±XX VT:[Code]±[Mag] -->
     ...
     E. SITUATION: <!-- SITUATION|[Label]|[TriggerCondition] -->
  ...
  5. NARRATIVE GOVERNANCE (USER-INITIATED ONLY)
     CRUX and SITUATION moments are NEVER auto-generated by the engine.
     
     When user sends (CRUX) command:
       → STOP narrative flow immediately.
       → ASSESS current character tensions (Want vs. Need).
       → GENERATE a CRUX based on current state.
       → PRESENT using CRUX format.

     When user sends (SITUATION) command:
       → INJECT an external pressure or event.
       → MUST output the SITUATION telemetry tag.
       → Describe the incident in prose and how it impacts the scene.

     DO NOT auto-generate CRUX or Situations based on stat thresholds. 
     The app tracks conditions; the user decides when to engage.
