...
const SimulationTerminal: React.FC<SimulationTerminalProps> = ({ messages, onSend, isProcessing, contextLoaded, onPlayTTS }) => {
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[#0a0a0a]">
        ...
        <p className="text-gray-500 max-w-md text-sm">
          Initialization required. Head to the <span className="text-blue-400 font-bold">Settings</span> tab to configure protocols.
        </p>
      </div>
    );
  }
        ...
        {narrativeMessages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] px-6 py-4 rounded-2xl transition-all relative group ${
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {msg.compliance && (
                      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter ${
                        ...
                        <span className="hidden sm:inline">{msg.compliance.isPassed ? 'Physics Compliant' : 'Engine Warning'}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="whitespace-pre-wrap leading-relaxed text-sm antialiased mb-2">
                {msg.content}
              </div>

              {msg.ooc && (
                <div className="narrative-ooc border-l-2 border-gray-700 pl-3 py-1 my-2 text-gray-500 italic text-[13px] leading-snug">
                  {msg.ooc}
                </div>
              )}
              ...
              {!msg.compliance?.isPassed && msg.compliance?.violations && msg.compliance.violations.length > 0 && (
                <div className="mt-4 p-3 bg-red-500/5 rounded-lg border border-red-500/10">
                  <p className="text-[10px] text-red-400 font-bold uppercase mb-1 flex items-center gap-2">
                    <AlertCircle size={10}/> Rule Violations:
                  </p>
